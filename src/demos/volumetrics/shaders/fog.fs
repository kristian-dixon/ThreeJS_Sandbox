...
bool sphereIntersection(vec3 ro, vec3 rd, vec4 sphereInfo, out float t0, out float t1)
{
...
void main()	{
    ...
    if(sphereIntersection(ro,rd,sphereParams,viewIntersectionNear, viewIntersectionFar))
    {
        float distTravelledThroughVolume = (viewIntersectionFar-viewIntersectionNear);
        ...
        for(float i = viewIntersectionNear; i < viewIntersectionFar - stepDistance * 0.5; i+=stepDistance)
        {
            ...
            if(sphereIntersection(samplePoint, lightDirNorm, sphereParams, t0,t1))
            {
