...
void main()	{ 
    ...
    float afterDepth = (1.0 - currentDepthMapValue.r) - currentLayerDepth;
    float beforeDepth = (1.0 - texture2D(NormalMap, prevTexCoords).r) - currentLayerDepth + perLayerDepth;
    ...
    float lit = max(dot(texture2D(AlbedoMap, currentTexCoords).rgb * 2.0 - 1.0, normalize(vec3(-0.2,0.3,1.0))), 0.0);
    gl_FragColor = vec4(vec3(1.0 * lit,1.0* lit,1.0* lit), 1.0);
